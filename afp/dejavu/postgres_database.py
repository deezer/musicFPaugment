...
import psycopg2
from dejavu.database import BaseDatabase
...
class CommonDatabase(BaseDatabase, metaclass=abc.ABCMeta):
    ...
    def insert_hashes(
        self, song_id: int, hashes: List[Tuple[str, int]], batch_size: int = 100
    ) -> None:
        """
    ...
    def return_matches(
        self, hashes: List[Tuple[str, int]], batch_size: int = 1
...
class PostgreSQLDatabase(CommonDatabase):
    ...
    CREATE_SONGS_TABLE = f"""
        ...
        ,   CONSTRAINT "pk_{"songs"}_{"song_id"}" PRIMARY KEY ("{"song_id"}")
        ,   CONSTRAINT "uq_{"songs"}_{"song_id"}" UNIQUE ("{"song_id"}")
        );
    """
    ...
    CREATE_FINGERPRINTS_TABLE = f"""
        ...
        ,   "date_created" TIMESTAMP NOT NULL DEFAULT now()
        ,   "date_modified" TIMESTAMP NOT NULL DEFAULT now()
        ,   CONSTRAINT "uq_{"fingerprints"}" UNIQUE  ("{"song_id"}", "{"offset"}", "{"hash"}")
        ,   CONSTRAINT "fk_{"fingerprints"}_{"song_id"}" FOREIGN KEY ("{"song_id"}")
                REFERENCES "{"songs"}"("{"song_id"}") ON DELETE CASCADE
        );

        CREATE INDEX IF NOT EXISTS "ix_{"fingerprints"}_{"hash"}" ON "{"fingerprints"}"
        USING hash ("{"hash"}");
    """

    CREATE_FINGERPRINTS_TABLE_INDEX = f"""
        CREATE INDEX "ix_{"fingerprints"}_{"hash"}" ON "{"fingerprints"}"
        USING hash ("{"hash"}");
    """
    ...
    SELECT_MULTIPLE = f"""
        SELECT upper(encode("{"hash"}", 'hex')), "{"song_id"}", "{"offset"}"
        FROM "{"fingerprints"}"
        WHERE "{"hash"}" IN (%s);
    """

    SELECT_ALL = f'SELECT "{"song_id"}", "{"offset"}" FROM "{"fingerprints"}";'

    SELECT_SONG = f"""
        SELECT
            "{"song_name"}"
        ,   upper(encode("{"file_sha1"}", 'hex')) AS "{"file_sha1"}"
        ,   "{"total_hashes"}"
        FROM "{"songs"}"
        WHERE "{"song_id"}" = %s;
    """

    SELECT_NUM_FINGERPRINTS = f'SELECT COUNT(*) AS n FROM "{"fingerprints"}";'

    SELECT_UNIQUE_SONG_IDS = f"""
        SELECT COUNT("{"song_id"}") AS n
        FROM "{"songs"}"
        WHERE "{"fingerprinted"}" = 1;
    """
    ...
    # DROPS
    DROP_FINGERPRINTS = f'DROP TABLE IF EXISTS "{"fingerprints"}";'
    DROP_SONGS = f'DROP TABLE IF EXISTS "{"songs"}";'
    ...
    DELETE_SONGS = f"""
        DELETE FROM "{"songs"}" WHERE "{"song_id"}" IN (%s);
    """
