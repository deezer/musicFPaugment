...
def upsample2(x: torch.Tensor, zeros: int = 56) -> torch.Tensor:
    """
...
def downsample2(x: torch.Tensor, zeros: int = 56) -> torch.Tensor:
    """
...
def rescale_conv(conv: Union[nn.Conv1d, nn.ConvTranspose1d], reference: float) -> None:
...
class Demucs(nn.Module):
    ...
    Args:
        ...
        causal (bool):
            If false, uses BiLSTM instead of LSTM.
        resample (int):
            Amount of resampling to apply to the input/output. Can be one of 1, 2 or 4.
        growth (float):
            Number of channels is multiplied by this for every layer.
        max_hidden (int):
            Maximum number of channels. Can be useful to control the size/speed of the model.
        normalize (bool):
            If true, normalize the input.
        glu (bool):
            If true uses GLU instead of ReLU in 1x1 convolutions.
        rescale (float):
            Controls custom weight initialization.
            See https://arxiv.org/abs/1911.13254.
        floor (float):
            Stability flooring when normalizing.
        sample_rate (float):
            Sample_rate used for training the model.
    """
    ...
    def valid_length(self, length: float) -> int:
        """
...
def fast_conv(conv: Union[nn.Conv1d, nn.ConvTranspose1d], x: torch.Tensor) -> Any:
    """
    ...
    if kernel == 1:
        ...
        out = torch.addmm(conv.bias.view(-1, 1), conv.weight.view(chout, chin), x)  # type: ignore
    elif length == kernel:
        x = x.view(chin * kernel, 1)
        out = torch.addmm(
            conv.bias.view(-1, 1), conv.weight.view(chout, chin * kernel), x  # type: ignore
        )
    else:
        out = conv(x)
...
class DemucsStreamer:
    ...
    Args:
        demucs (Demucs):
            Demucs model.
        dry (float):
            Amount of dry (e.g. input) signal to keep. 0 is maximum
            noise removal, 1 just returns the input signal. Small values > 0
            allows to limit distortions.
        num_frames (int):
            Number of frames to process at once. Higher values
            will increase overall latency but improve the real time factor.
        resample_lookahead (int):
            Extra lookahead used for the resampling.
        resample_buffer (int):
            Size of the buffer of previous inputs/outputs kept for resampling.
    """
    ...
    def feed(self, wav: torch.Tensor) -> torch.Tensor:
        """
    ...
    def _separate_frame(self, frame: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        demucs = self.demucs
        skips = []
        next_state = []
        ...
        return x[0], extra[0]  # type: ignore
...
if __name__ == "__main__":
    ...
    sr = args.sample_rate
    sr_ms = sr / 1000
    demucs = Demucs(hidden=args.hidden, depth=args.depth, resample=args.resample).to(  # type: ignore
        args.device
    )
    x = torch.randn(1, int(sr * 10)).to(args.device)
    out = demucs(x[None])[0]
    ...
    with torch.no_grad():
        while x.shape[1] > 0:
            out_lst.append(streamer.feed(x[:, :frame_size]))
            x = x[:, frame_size:]
            frame_size = streamer.demucs.total_stride
    out_lst.append(streamer.flush())
    out_rt = torch.cat(out_lst, 1)
    ...
    print(f"delta batch/streaming: {torch.norm(out - out_rt) / torch.norm(out):.2%}")  # type: ignore
